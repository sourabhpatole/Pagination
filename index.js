...
let currentPage =0;
let pageSize =5;
let totalPages=userData.length/5
function onPageChangeHandler (page){
  if (page=="n"&& currentPage!=totalPages){
    currentPage++;
    updateTable()
  }
 else if (page=="p"&&currentPage!=0){
    currentPage--;
    updateTable()
  }
  else{
...
function updateTable (){
  ...
  for(let i=currentPage*pageSize;i<currentPage*pageSize+5;i++){
    insertRow(tableBody,userData[i].id,userData[i].name,userData[i].email)
  }

} 
function insertRow(table,val1,val2,val3){
...
cell0.innerHTML=val1

cell1.innerHTML=val2
